...
if numel(reportTable)>1
    report_mat=zeros(1,size(reportTable(1).qMats.MeasList,1)/2);
    master_threshold = reportTable(1).qMats.thresholds.quality;
    for i =1:numel(reportTable)
        report_mat = report_mat + (reportTable(i).qMats.good_combo_link(:,3)>=master_threshold)';
    end
    ...
    report_mat = reportTable.qMats.good_combo_link(:,3);
